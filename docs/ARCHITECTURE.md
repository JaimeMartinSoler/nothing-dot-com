...
- `/src/sentenceLists.js`: Pure, DOM-free selection logic (alias parsing, repetition tracking, candidate ordering). Kept separate from `main.js` so it can be unit tested.
...
- **Repetition Tracking**: Shown list aliases (the filename without extension, e.g. `000001`) are stored in `localStorage` under `nothing_shown_lists`. Selection draws only from lists not yet shown; once all have been shown the history resets and starts over. When the history is empty (first visit or a freshly reset cycle), the `start_with_first_list` flag in `config.yml` decides whether the first list (`000000`) is shown deterministically or a random one is picked; every subsequent pick is random.
- **Event Handling**:
