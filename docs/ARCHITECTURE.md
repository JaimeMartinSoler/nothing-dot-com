...
- `/src/config.yml`: The source of truth for app configuration (like the initial delay and cooldown delay), `visuals`, and the optional `subtitle-begin` and `subtitle-end` blocks. Sentences no longer live here.
- `/src/sentences/`: One YAML list of sentences per file (`000.yaml`, `001.yaml`, ...). The filename (sans extension) is the list's stored alias. One list is selected per page load.
...
- `/src/subtitle.js`: Pure, DOM-free logic for the optional configurable subtitles (resolving the per-language text, deciding whether to show it, resolving the extra delay). The `subtitle-begin` and `subtitle-end` blocks in `config.yml` are entirely optional; this module returns safe defaults (e.g. `null` text) when they are absent or incomplete. Kept out of `main.js` so it can be unit tested in isolation.

## Core Mechanisms
- **YAML Parsing & CSS Injection**: Uses `@modyfi/vite-plugin-yaml` to parse `config.yml`. `main.js` then reads the `visuals` block and dynamically injects them as CSS Custom Properties (`--bg-color`, `--blur-amount`, etc.) onto the document's `:root`. The optional `subtitle-begin` and `subtitle-end` blocks are injected the same way (`--sub-begin-text-color`, `--sub-end-font-size`, etc.), each variable falling back to its main-text counterpart when left null.
