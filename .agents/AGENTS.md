...
7. **YAML Config**: The timing behavior and ALL visual configurations (colors, fonts, animation timings, easing curves) must be stored in `config.yml`. Sentence text content lives in its own per-list files under `src/sentences/` (e.g. `000.yaml`), one list per file. This allows the site to be fully customized without touching JS/CSS source code. Each sentence's per-language value is either a plain string or an array of strings; an array is a *progressive sub-sentence* whose parts reveal one click at a time (e.g. `["Or keep clicking.", " It's your time"]`).
8. **Keep logic testable**: DOM-free logic that can be unit tested should live in its own pure module (e.g. `src/sentenceLists.js`, `src/subSentences.js`) rather than inside `src/main.js`, which touches the DOM at import time and is not under test.
